...
function loadDatabase(){
  let readingDatabase = null;
  if(fs.existsSync(databaseFilePath)){
    readingDatabase = yaml.safeLoad(fs.readFileSync(databaseFilePath, 'utf8'));
  } else{
...
function createComment(url, request){
  ...
  if (requestComment && requestComment.body && requestComment.articleId && database.users[requestComment.username] && database.users[requestComment.username] && requestComment.articleId && database.articles[requestComment.articleId]){
    const comment = {
